...
### 1.4 f2dnrq (NOTAM) のSec-Fetch-*除外

f2dnrqサービス（デジタルノータムリクエストサービス）はf2asprと異なるWAF設定を持ち、`Sec-Fetch-Dest` / `Sec-Fetch-Mode` / `Sec-Fetch-Site` ヘッダーが存在すると404を返す。f2dnrq向けリクエスト時のみこれらのヘッダーを除外する。
...
Chrome TLSフィンガープリントとその他のヘッダー（Accept, Origin, X-Requested-With等）は維持される。
