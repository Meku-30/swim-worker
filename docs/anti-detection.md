...
SWIMポータルはjQueryとAngularが混在しており、リクエスト種別によってヘッダーパターンが異なる。2026-04-06のPlaywrightキャプチャで実測し、4つのパターンを特定した。

#### ヘッダーパターン（2026-04-06 Playwrightキャプチャ実測）

| パターン | 対象リクエスト | Origin | X-Requested-With | Accept |
|---------|--------------|--------|-----------------|--------|
| **Document** | ブラウズ画面GET | なし | なし | `text/html,...` |
| **jQuery XHR** | settings/*.json, ATCMAP.settings, browse追加GET, LuciadRIALicense POST | POST時のみ | `XMLHttpRequest` | GET: `application/json, text/javascript, */*; q=0.01`、LuciadRIA/ATCMAP: `text/plain, */*; q=0.01` |
| **Angular resource** | web/resource/message, webfw, user | なし | なし | `*/*` |
| **Angular API** | web/FLV901等 POST、ログインPOST | あり | なし | `application/json, text/plain, */*` |

#### UTMレイヤー（LuciadRIA地図ライブラリ）

UTM*.jsonはLuciadRIAの地図ライブラリから読み込まれ、独自のAcceptヘッダーを持つ:
- `Accept: application/javascript, application/json`
- `Origin`: なし、`X-Requested-With`: なし
...
`Origin`と`X-Requested-With`はセッションデフォルトに含めない。ナビゲーションGETとのマージ時に残留してbot判定されるリスクを回避するため。各リクエスト種別で個別に付与する。
...
`Cache-Control` / `Pragma` は含めない。Chromeは通常のナビゲーションでこれらを送信しない（Ctrl+Shift+R 強制リロードのみ）。
...
```
1. GET https://top.swim.mlit.go.jp/swim/  (NAV_HEADERS, Sec-Fetch-Site: none)
2. 1-3秒ランダム待機（SPA読み込み時間を再現）
3. POST https://top.swim.mlit.go.jp/swim/api/login  (SESSION_HEADERS + Origin + Referer: /swim/login)
     body: {"userId": "...", "password": "..."}
4. 0.5-1.5秒ランダム待機（リダイレクト遅延を再現）
5. GET https://web.swim.mlit.go.jp/service/portal?lang=ja  (NAV_HEADERS, Sec-Fetch-Site: same-site, Sec-Fetch-User: ?1)
     遷移先はログインレスポンスの redirectUrl に従う
6. 0.5-1.5秒ランダム待機
7. 以降、web.swim.mlit.go.jp へのXHRでAPI呼び出し
```

ステップ3のヘッダー: Origin は `https://top.swim.mlit.go.jp`、Referer は `https://top.swim.mlit.go.jp/swim/login`。X-Requested-With は付与しない（Angular HttpClient は送信しない、2026-04-06実測確認）。

ステップ5が重要: 実ブラウザではログイン後にSPAが `web.swim` へリダイレクトする。これがないと「`web.swim` へのdocumentナビゲーションなしにXHRが来た」と検知される可能性がある。`top.swim` → `web.swim` はサブドメイン間遷移のため `Sec-Fetch-Site: same-site` が正しい。Sec-Fetch-User は `?1`（ユーザー起因のナビゲーション、2026-04-06実測確認）。

ログインフロー中の全Cookie（ページGET + ログインPOST + web.swim遷移）を永続セッションに移す。Cookie domain は `.mlit.go.jp`（2026-04-06実測確認）。保存済みCookieでセッション復元できる場合はこのフロー全体をスキップし、不要なアクセスを削減する。
...
| レスポンス後遅延 | APIレスポンス受信後 | 指数分布（中央値~0.28秒、稀に1-2秒） | ブラウザのDOM更新・レンダリング時間を再現。均一分布より自然なばらつき |
...
- ログイン成功後、セッションCookie（domain: `.mlit.go.jp`、2026-04-06実測確認）をJSONファイルに保存
- 保存先は環境に応じて自動決定:
  - Docker: `/app/data/.swim_cookies.json`
  - PyInstaller exe: exe と同じディレクトリの `data/`
  - VPS/systemd: ワーキングディレクトリの `data/`
  - 環境変数 `COOKIE_FILE` でオーバーライド可能
...
| ジョブ種別 | 最大ジッター | 理由 |
|-----------|-------------|------|
| 通常ジョブ（NOTAM, PIREP, PKG Full/Half, フライト詳細等） | 30秒 | トリガー間隔が10分以上 |
| PKG高頻度収集 (HF) | 10秒 | 5分間隔のため長いジッターでは次トリガーと衝突 |

```
設定: JOB_JITTER_SECONDS=30（デフォルト、HFは10秒固定）
```
...
NOTAMジョブはタスクを逐次dispatch（各タスク間に15-40秒のランダムスリープ）で分散する。同一セッションから短時間に複数の同時リクエストが飛ぶパターンを避けるため。

PKG気象・フライト詳細はWorker数分の並列dispatch（各Workerが異なるIP/セッションで処理するため、同一セッションからの並列リクエストにはならない）。
...
```python
async with AsyncSession(impersonate=BrowserType.chrome136, timeout=15) as client:
...
**注**: 上記はCoordinatorがdispatchしたデータ収集タスク数。Worker内部で発生する以下のオーバーヘッドは含まれていない:

| 種別 | req/セッション | 発生頻度 | 24h推定 |
|------|--------------|---------|---------|
| SPA初期化 (f2aspr+f2dnrq) | ~52 | ログイン/再ログインごと | ~100-200 |
| ログインフロー | ~3 | ログイン/再ログインごと | ~6-18 |
| Cookie復元確認 | ~2 | プロセス起動時 | ~4-6 |

オーバーヘッド込みの実総リクエスト: **~5,300-5,400/日**
